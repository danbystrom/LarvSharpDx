...
namespace Larv.Field
{
    public class PlayingFieldBuilder
    {
            ...
            for (var y = 0; y < height; y++)
                for (var x = 0; x < width; x++)
                    switch (field[y][x])
                    {
                        ...
                        case 'D':
                            _field[floor, y, x] = createPortalSquare(x, y, field);
                            break;
                    }
            ...
            while (true)
            {
                ...
                for (var y = 0; y < height; y++)
                    for (var x = 0; x < width; x++)
                    {
                        ...
                        {
                            case 'U':
                                _field[floor, y, x] = createSlopeSquare(x, y);
                                if (_field[floor, y, x].IsSlope)
                                    continue;
                                mustRunAgain = true;
                                break;
                        }
        ...
        private PlayingFieldSquare createPortalSquare(int x, int y, string[] field)
        {
            foreach (var direction in Direction.AllDirections)
            {
                if (getSquareChar(direction.DirectionAsPoint().Add(x, y), field) == ' ')
                    continue;
                return new PlayingFieldSquare(
        ...
        private char getSquareChar(Point p, string[] field)
        {
            if (p.Y < 0 || p.Y > _field.GetUpperBound(1))
                return ' ';
            if (p.X < 0 || p.X > _field.GetUpperBound(2))
